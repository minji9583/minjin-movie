...
def list(request):
    ...
    if len(score_movies) == 0:
        ...
        pref_genre = Genre.objects.get(pk=genre_max_value_count)
        genre_reco_movies = []
        ...
        pref_actor = Actor.objects.get(pk=actor_count.index(actor_max_value))
        actor_reco_movies = []
    ...
    def sim_pearson(users, name1, name2):
        ...
        if count == 0: return 0
            ...
            denominator = math.sqrt( (sumPowX - (pow(sumX,2) / count)) * (sumPowY - (pow(sumY,2)/count)))
            if denominator == 0:
                return (sumXY - ((sumX*sumY)/count))
            else:
                return (sumXY - ((sumX*sumY)/count))/ denominator
    ...
    print(result_list)
    #### 여기까지
    context = {'movies': movies, 'genre_reco_movies':genre_reco_movies, 'pref_actor': pref_actor, 'pref_genre': pref_genre, 'result_list': result_list, 'actor_reco_movies':actor_reco_movies, 'genres':genres}
...
def boxoffice(request):
    ...
    movie_list = {}
    for rank in response['boxOfficeResult']['weeklyBoxOfficeList']:
        try:
            movie_id = Movie.objects.get(title=rank['movieNm'])
            img_path = movie_id.poster_url
        except ObjectDoesNotExist:
            movie_id = 0
            naver_url = 'https://openapi.naver.com/v1/search/movie.json?query={0}'.format(rank['movieNm'])
            ...
            img_path = soup.select_one('#targetImage')
        movie_list.update({rank['movieNm']: {
            'id': movie_id,
            'title': rank['movieNm'],
            'opening': rank['openDt'],
            'amt': rank['audiAcc'],
            'share': rank['salesShare'],
            'img_path': img_path
        }})
    context = {'response': response, 'movie_list': movie_list}
